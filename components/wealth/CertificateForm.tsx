  ...
  return (<div className="glass" style={{padding:'1rem',borderRadius:12}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:8}}>
      ...
      <select className="input" value={currency} onChange={e=>setCurrency(e.target.value as Currency)}>
        {['KWD','EGP','USD','EUR','SAR','AED','QAR'].map(c=><option key={c} value={c}>{c}</option>)}
      </select>
    ...
    <div style={{display:'flex',gap:8,marginTop:8}}>
      ...
      <button className="btn btn-outline" onClick={()=>{ setTitle('Savings Certificate'); setPrincipal(0); setRateAPR(0); setStartDate(''); setEndDate(''); setPayoutDay(15); }}>Reset</button>
    </div>
  </div>);
}
