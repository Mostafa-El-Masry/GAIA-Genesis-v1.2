    ...
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm border">
        <thead className="bg-gray-200"><tr>
          <th className="p-2 border">Title</th><th className="p-2 border">Curr</th><th className="p-2 border">Principal</th><th className="p-2 border">APR%</th><th className="p-2 border">Start</th><th className="p-2 border">End</th><th className="p-2 border">Next</th><th className="p-2 border">Actions</th>
        </tr></thead>
        <tbody>
          {items.map(c=>{ const upcoming=buildSchedule(c).filter(s=>s.status!=='past'); const next=upcoming[0];
            return (<tr key={c.id} className="border-t">
              <td className="p-2 border"><button className="text-blue-600 hover:underline" onClick={()=>onOpen(c.id)}>{c.title}</button></td>
