...
export default function VideoGallery({ items, enableSelection=false, onToggleSelect, isSelected }:{ items:VideoItem[]; enableSelection?:boolean; onToggleSelect?:(src:string)=>void; isSelected?:(src:string)=>boolean; }){
  ...
  useEffect(()=>{
    // load the new source explicitly (some browsers need it)
    const v = playerRef.current;
    if(v){ v.pause(); v.load(); }
  }, [current?.src]);

  const onBigVideoClick = () => {
    const v = playerRef.current;
    if(!v) return;
    ...
    <div style={{border:'1px solid rgba(255,255,255,.12)',borderRadius:12,background:'#0f1117',padding:6}}>
      {current ? (
        <video
          ...
          onClick={onBigVideoClick}
        />
      ) : <div style={{padding:'1rem'}}>No video selected</div>}
    </div>
    <div style={{display:'grid',gridTemplateColumns:'repeat(5,minmax(0,1fr))',gap:12}}>
      {items.map((v,i)=>(
        <div key={v.src+i} role="button" tabIndex={0}
             onClick={()=>setCurrent(v)}
             style={{position:'relative',border:'1px solid rgba(255,255,255,.12)',borderRadius:12,overflow:'hidden',background:'#0f1117'}}>
