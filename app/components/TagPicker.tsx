...
export default function TagPicker({ value, onChange, suggestions = [], placeholder = 'Add tag and Enter' }:{ value:string[]; onChange:(v:string[])=>void; suggestions?:string[]; placeholder?:string; }){
  const [input, setInput] = useState('');
  const add = (t:string) => { const tag=t.trim(); if(!tag || value.includes(tag)) return; onChange([...value, tag]); setInput(''); };
