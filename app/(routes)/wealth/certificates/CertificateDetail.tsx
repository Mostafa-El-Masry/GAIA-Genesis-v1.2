...
export default function CertificateDetail({ id, onClose }:{ id:string|null; onClose:()=>void }){
  ...
  return (<div className="glass" style={{padding:'1rem',borderRadius:12}}>
    ...
    <div className="overflow-x-auto"><table className="min-w-full text-sm border">
      ...
      <tbody>
        {schedule.map(s=>(<tr key={s.date} className="border-t" style={{opacity: s.status==='past'?0.6:1}}>
        ...
        {schedule.length===0 && <tr><td colSpan={3} className="text-center text-gray-400 p-3">No schedule.</td></tr>}
      </tbody>
    </table></div>
    ...
    <div className="overflow-x-auto" style={{marginTop:8}}><table className="min-w-full text-sm border">
      ...
      <tbody>
        {receipts.map(r=>(<tr key={r.date} className="border-t"><td className="p-2 border">{r.date}</td><td className="p-2 border">{r.amount.toFixed(2)}</td><td className="p-2 border"><button className="btn" onClick={()=>deleteReceipt(cert.id, r.date)}>Delete</button></td></tr>))}
